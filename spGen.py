...
def transportToInverseGaussian(length, velocity, dispersion):
    ...
    """
    return length/velocity, length*length/(2.*dispersion)
...
for folder_name in folder_names_list:
    ...
    n_atoms_dissolved_introduced = rngd.poisson(params.flux_dissolved*params.dwell_time, params.n_reads)
    n_ions_detector = rngd.binomial(n_atoms_dissolved_introduced, params.f_transmission, params.n_reads)
    ...
    ppjumps = poissonProcess(params.flux_particles*params.dwell_time, params.n_reads)
    ...
    if(params.size_distribution == "dirac"):
        ...
        diameters = rngp.normal(params.size_particle_mean, params.size_particle_std, n_particles)
        diameters[diameters < 0.] = 0.    # get rid of < 0 sizes
    elif(params.size_distribution == "lognormal"):
        mu_ln = 2*math.log(params.size_particle_mean) - 0.5*math.log(params.size_particle_mean**2 + params.size_particle_std**2)
        sigma_ln = math.sqrt(math.log(1 + params.size_particle_std**2 / params.size_particle_mean**2))
        diameters = rngp.lognormal(mu_ln, sigma_ln, n_particles)
    atoms_per_particle = diameters**3 * (PI/6) * params.mass_density * params.molar_fraction * params.isotopic_abundance * AVOGADRO / params.molar_mass
    atoms_per_particle = (atoms_per_particle).astype(int)   # array of floats -> array of ints
    n_atoms_particles_detected = rngp.binomial(atoms_per_particle, params.f_transmission, n_particles)
    n_atoms_particles_detected_nonzero = n_atoms_particles_detected[n_atoms_particles_detected != 0]
    ...
    for i in range(n_particles):
        ...
        if(params.shape_distribution == "inverse-gaussian"):
            time = ppjumps[i]*np.ones(n_atoms_particles_detected[i]) +\
                rngp.wald(params.mean/params.dwell_time, params.scale/params.dwell_time, n_atoms_particles_detected[i])
        # uniform distribution of the spike duration
        elif(params.shape_distribution == "uniform"):
            time = ppjumps[i]*np.ones(n_atoms_particles_detected[i]) +\
                rngp.uniform(0., 2*params.mean/params.dwell_time, n_atoms_particles_detected[i])
        time = np.rint(time)
        if(time.size > 0):
            particle_event_duration[i_particle_event] = int(max(time))-int(min(time))+1
    ...
    if(params.detector == "ideal"):
        ...
        for i in range(params.n_reads):
            if(timescan_predetect[i] > 0):
                ...
                timescan[i] = np.sum(rngo.lognormal(params.sir_mean, params.sir_std, timescan_predetect[i].astype(int)))

    # Output yaml file
    if (n_particle_events > 0):
        output_dict = {'datetime': datetime.datetime.now(), 'id': params.id, 'code_name': __codename__, 'code_version': __version__,\
                    'n_particles_introduced': n_particles,\
                    'n_particle_events': n_particle_events,\
                    'particle_event_cumulative_counts_mean': float('{:.3e}'.format(np.mean(n_atoms_particles_detected_nonzero))),
                    'particle_event_cumulative_counts_std': float('{:.3e}'.format(np.std(n_atoms_particles_detected_nonzero))),
                    'particle_event_duration_mean': float('{:.3e}'.format(np.mean(particle_event_duration)*params.dwell_time)),
                    'particle_event_duration_std': float('{:.3e}'.format(np.std(particle_event_duration)*params.dwell_time))
                    }
    else:
        output_dict = {'id': params.id,\
                    'n_particles_introduced': n_particles,\
                    'n_particle_events': 0,\
                    'particle_event_duration_mean': 0.,
                    'particle_event_duration_std': 0.,
                    'particle_event_cumulative_counts_mean': 0.,
                    'particle_event_cumulative_counts_std': 0.
                    }
    ...
    if(params.timescan_csv):
        ...
        with open(tra_path, mode='w', newline='\n') as file:
            ...
            for index, item in enumerate(timescan):
                writer.writerow([round((index+1)*params.dwell_time + params.timescan_csv_start, TIME_PREC), \
                                item])
    ...
    with open(event_path, mode='w', newline='\n') as file:
        ...
        for i in range(n_particles):
            if(n_atoms_particles_detected[i] > 0):
                writer.writerow([i_particle_event+1, \
                                 round(particle_event_start[i_particle_event]*params.dwell_time + params.timescan_csv_start, TIME_PREC), \
                                     round(particle_event_end[i_particle_event]*params.dwell_time + params.timescan_csv_start, TIME_PREC)])
